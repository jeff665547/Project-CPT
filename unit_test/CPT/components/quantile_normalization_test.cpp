...
TEST(component_test, quantile_normalization)
{
    /* prepare option */
    auto in_path =
        nucleona::language::sdir(__FILE__)
        #ifdef NEW_DATA_POOL
        + "/quantile_normalization_test/in_d2.json"s
        #else
        + "/quantile_normalization_test/in.json"s
        #endif
    ;
    Option option{ 
    ...
    std::cout << "final symbol table info" << std::endl;
    cpt::engine::data_pool::component_object_manager::CPSymTab::show_info( std::cout );
