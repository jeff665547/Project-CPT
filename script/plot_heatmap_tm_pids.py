...
# read pids file, file only contains one column and no header
pids = pd.read_csv(pidin, header=None, names=['pid'])

# Read input csv file
df1 = pd.read_csv(input)

# if df1['pid'] is not in pids['pid'], replace df1['tm'] with 0
df1.loc[~df1['pid'].isin(pids['pid']), 'tm'] = 0
df2 = df1.copy()
...
fig.suptitle(pidin.split('.')[0], fontsize=14)

# Save to png
fig.tight_layout()
fig.subplots_adjust(top=0.93)
plt.savefig(pidin.split('.')[0] + '.png', bbox_inches="tight")
