...
int main(int argc, char **argv)
{
    ...
    (",p", boost::program_options::value<std::string>()->required(), "Set input pid file list")
    (",v", boost::program_options::value<std::string>()->required(), "Set input vcf file list")
    (",f", boost::program_options::value<std::string>()->required(), "Set input gnome.fna file")
    (",i", boost::program_options::value<std::string>()->required(), "Set input anno.csv file")
    (",o", boost::program_options::value<std::string>()->required(), "Set output file name");
    try
    {
    ...
    std::ifstream inpbl(op["-p"].as<std::string>());
    ...
    std::set<std::string> biall_ps, np_ps;
    ...
    while (std::getline(input, temp))
    {
        ...
        if(probes.find(probe_id) == probes.end())
            continue;
    ...
    boost::property_tree::json_parser::write_json(output + "_filterd.json", json);

    std::ofstream biall_out("biallelic_filtered.ps");
