...
int main(int argc, const char *argv[])
{
    if (argc != 3)
    {
        ...
        std::cout << "probeset_id should be in the format of [probeset_id]_[probe_id]" << std::endl;
        return 1;
    }
    ...
    auto psids_idx = std::map<std::string, std::map<std::string, int>>();
    ...
    while (std::getline(call_file, line))
    {
        ...
        boost::iter_split(splited, call_id, boost::algorithm::first_finder("_"));

        auto &psid = splited[0];
        auto &pid = splited[1];

        psids_idx[psid].emplace(pid, 0);
    ...
    auto ps_ids = std::vector<std::string>();
    ...
    for (auto &psid : psids_idx)
    {
        ...
        for (auto &pid : psid.second)
        {
            pid.second = idx;
            ps_ids.emplace_back(psid.first + "_" + pid.first);
            idx++;
        }
    }
    ...
    auto gt_vcf = std::vector<std::vector<int>>(ps_ids.size(), std::vector<int>(names_idx.size(), -1));  // GT: -1=./., 0=0/0, 1=0/1, 2=1/1
    auto gt_call = std::vector<std::vector<int>>(ps_ids.size(), std::vector<int>(names_idx.size(), -1)); // GT: -1=./., 0=0/0, 1=0/1, 2=1/1
    auto gt_acc = std::vector<std::vector<int>>(ps_ids.size(), std::vector<int>(names_idx.size(), -1));  // ACC: -1=no call, 0=fasle, 1=true

    for (idx = 0; idx < call_gts.size(); idx++)
    {
        std::cerr << "Assigning genotype call: " << idx + 1 << "/" << call_gts.size() << "\r";
        auto &call_id = call_psids[idx];
        ...
        gt_call[psids_idx[psid][pid]][names_idx[name]] = gt;
    }
    ...
    for (auto &cen : names_idx)
    {
        ...
        if (mappings.find(cen_name) != mappings.end() && mappings[cen_name] != "")
        {
            ...
            while (std::getline(filein, line))
            {
                ...
                for (auto &pid : psids_idx[vcf_id])
                {
                    auto &call_gt = gt_call[psids_idx[vcf_id][pid.first]][names_idx[cen_name]];
                    boost::iter_split(splited, vcf_gt, boost::algorithm::first_finder("/"));

                    if (splited[0] == "1")
                    {
                        gt_vcf[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 2;

                        switch (call_gt) // 1/1
                        {
                        case 0:
                            gt_acc[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 0;
                            break;
                        case 1:
                            gt_acc[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 0;
                            break;
                        case 2:
                            gt_acc[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 1;
                            break;
                        }
                    }
                    else if (splited[0] == "0")
                    {
                        if (splited[1] == "0")
                        {
                            gt_vcf[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 0;
                        ...
                        {
                            gt_vcf[psids_idx[vcf_id][pid.first]][names_idx[cen_name]] = 1;
