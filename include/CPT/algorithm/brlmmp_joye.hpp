...
class BRLMMpClustering
{
  public:
    ...
    //                        Sum      Likelihood
    BRLMMpClustering( const std::vector< double >& data, const size_t& n_cluster ) 
        : datas             ( data )
    ...
    BRLMMpClustering( const BRLMMpClustering& clustering ) 
        : datas             ( clustering.datas )
        , last_means        ( clustering.last_means )
        , last_variances    ( clustering.last_variances )
        , last_boundarys    ( clustering.last_boundarys )
        , skip_n            ( clustering.skip_n )
        , buffer            ( clustering.buffer )
        , likelihoods       ( clustering.likelihoods )
    {
    }
  ...
  public:
    ...
    void outlier_trimming_for_1_cluster( BRLMMpBufferType& buffer )
    {
        ...
        if( !outlier_idx.empty() )
        {
            ...
            BRLMMpClustering brlmmp( data, 1 );
    ...
    void outlier_trimming_for_2_cluster( BRLMMpBufferType& buffer )
    {
        ...
        if( !outlier_idx.empty() )
        {
            ...
            BRLMMpClustering brlmmp( data, 2 );
            std::pair< double, BRLMMpBufferType > likelihood( brlmmp.run( this->brlmmp_type_ ));
            // buffer.clear();
            // buffer.swap( likelihood.second );
            buffer = std::move( likelihood.second );
    ...
    void find_min_bic( const std::vector< double >& data )
    {
        ...
        {
            BRLMMpClustering brlmmp( data, k+1 );
    ...
    )
    {
        ...
        for( size_t i = 0; i < k; ++i )
        {
            vec.emplace_back( mean_or_std[i] - mean_of_mean_or_std[i] );
        }
    ...
    {
        imputate_for_1_cluster( missing_k, this->means_of_mean_, this->variances_of_mean_, mean_or_std, mean_of_mean_or_std );
    }
    ...
    {
        imputate_for_2_cluster( given_k, this->means_of_mean_, this->variances_of_mean_, mean_or_std, mean_of_mean_or_std );
    }
