...
class MinusVsAverage : public engine::NamedComponent
{
    ...
    double k_;
    /* input */
    com_::ICPObjPtr< std::string > mva_prefix_;
    ...
    com_::ICPObjPtr< std::vector<std::string> > mva_allele_signals_; /* output */
  ...
  protected:
    ...
    virtual void config_parameters( const bpt::ptree& p ) override
    {
        ...
        tsvs_ = com_::require_w< Tsvs >( 
              json, "minus_vs_average"
            , com_::make_ref_parameter("MinusVsAverage") 
        );
        probeset_cube_ = com_::require_w<cpt::format::Cube<double>>(
            json, "probeset_cube", com_::make_ref_parameter("probeset_cube")
        );

        mva_prefix_ = com_::require_w< std::string >(
            json, "mva_prefix", com_::make_ref_parameter("output_dir")
        );
        mva_allele_signals_ = com_::require_w< std::vector<std::string> >(
              json, "mva_allele_signals"
            , com_::make_ref_parameter("mva_allele_signals")
        );
  ...
  public:
    ...
    virtual void initialize() override
    {
        ...
        mva_prefix_         -> initialize();
        mva_allele_signals_ -> initialize();
    }
    ...
    virtual void start() override
    {
        ...
        auto& mva_prefix = mva_prefix_ -> get();
        auto& mva_allele_signals = mva_allele_signals_->get();
        auto& tsvs = tsvs_->get();


        monitor.set_monitor( "Component Minus vs Average Transformation", 2 );
        monitor.log( "Component Minus vs Average Transformation", "Start ... " );

        // row: allele (xy), col: sample,     slice: pb
        probeset_cube.rotate_dimension( std::vector< size_t >({ 2, 1, 0 }));
        // row: pb,          col: sample,     slice: allele (xy)
        ...
        if( is_tsv_ )
        {
            ...
            setup_tsv( tsvs, mva_prefix, mva_allele_signals, probeset_cube );
            ...
            res_output( tsvs, mva_allele_signals );
        ...
        monitor.log( "Component Minus vs Average Transformation", "Complete!!!" );
    }
    template<class CUBE>
    void setup_tsv(
          Tsvs& tsvs
    ...
    {
        auto& db( this->mut_data_pool() );
        auto& node = db.require_output( "mva_transformation" );
        ...
        for( auto& sample : probeset_cube.y_axis.get_labels() )
        {
            ...
            auto path = prefix;
            path += sample + ".MinusVsAverage.tsv";

            db.add_output_path( node, path );
            paths.push_back( path );
        }

        // db.assign_path( "mva_transformation", paths );
    }
    ...
    void res_output(Tsvs& tsvs, const std::vector<std::string>& path_list )
    {
        ...
        // auto& paths = db.get_path_list( "mva_transformation" );
