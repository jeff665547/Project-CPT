#pragma once
#ifdef NO_PT // for O0 bug
#include <CPT/components/birdseed/dummy_probeset_training.hpp>
#else

#ifdef NEW_DATA_POOL
#include <CPT/components/birdseed/probeset_training2.hpp>
#else
  ...
  public:
    ...
    virtual void config_parameters( const bpt::ptree& p ) override
    {
        ...
        select_probeset_num = json
            .get_optional<std::size_t>("select_probeset_num")
            .value_or(0);

        db.require( grand_model , "GrandModel" );

        algo_params = json.get_child("fitting_algo").root;

        archive      = json
            .get_optional<std::string>("archive")
            .value_or("");
        cpt::dbg << "archive : " << archive << std::endl;
        db.thread_pool->resize_pool(json.get_optional<std::size_t>("thread_num").value_or(16));
        auto dvlp = json.get_optional<std::string>("dbg_view_list");
        if ( dvlp )
        {
    ...
    virtual void initialize() override
    {
        ...
        parameter_check();
        auto& db( this->mut_data_pool() );
        db.models.resize(db.cube.n_slices);
        db.validities.resize(db.cube.n_slices, false);
    ...
    virtual void NOINLINE start() override
    {
        ...
        | ba::transformed(
            ...
            ]( auto mid )
            {
                ...
                const auto& grand_model_v( grand_model.get() );
                std::vector<GMMean> g_m_means  {
                      GMMean {
                          grand_model_v.Component(0).Mean()
                      }
                    , GMMean {
                          grand_model_v.Component(1).Mean()
                      }
                    , GMMean {
                          grand_model_v.Component(2).Mean()
                      }
                    , GMMean {
                          grand_model_v.Component(0).Mean()
                        , grand_model_v.Component(2).Mean()
                ...
                //     5, g_m_means.size(), grand_model_v, logger
                // );
                ...
                //     , cu::ThreadDurationLabel::parallel_mode
                // ) );
                auto mapper ( this->norm_component_pos ( best_mdl, grand_model_v ) );
                ...
                dbmodel = best_mdl;
                {
                    ...
                    #ifdef PROBESET_TRAINING_PLOT
                    std::lock_guard<std::mutex> lock(monitor_mux);
                    auto ps_name = db.cube.z_axis.search_label_at(mid);
